...
export default function AddShapes(props)
{
  ...
  const [addPicture,setAddPicture]=useState({filename:"",bytes:""});

  

  const handlePicture=(event)=>{
setAddPicture({filename:URL.createObjectURL(event.target.files[0]),bytes:event.target.files[0]})
  }
...
if(isEmpty(addPicture.filename)){
  err=true;
...
if(!err){
    var formData=new FormData();
      ...
      formData.append("addpicture",addPicture.bytes);
...
<Grid item xs={6} style={{display:"flex",justifyContent:"center",alignItems:"center" }}>
<Avatar alt="Remy Sharp" variant="rounded" src={addPicture.filename} style={{width:50, height:50}} />
</Grid>
<Grid item sm={12}>
